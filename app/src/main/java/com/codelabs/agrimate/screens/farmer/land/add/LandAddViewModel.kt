...
@HiltViewModel
class LandAddViewModel @Inject constructor(
    private val farmerLandUseCase: FarmerLandUseCase,
    private val regionUseCase: RegionUseCase,
) : ViewModel() {
    private val _uiState = MutableStateFlow(LandAddUiState())
    val uiState: StateFlow<LandAddUiState> = _uiState.asStateFlow()
    ...
    fun saveLand() {
        viewModelScope.launch {
            ...
            if (isFormValid()) {
                _uiState.value.run {
                    farmerLandUseCase.createFarmerLand(
